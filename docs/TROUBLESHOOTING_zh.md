...
1.1 **检查mcp-chrome-beta-bridge是否安装成功**，确保是**全局安装**的
...
> 如果发现没有此清单文件，可以尝试命令行执行：`mcp-chrome-beta-bridge register`
...
或者运行 `mcp-chrome-beta-bridge doctor --fix` 来写入当前 Node 路径。
