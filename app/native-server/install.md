...
```
npm install -g mcp-chrome-beta-bridge
└─ postinstall.js
   ├─ 复制可执行文件到 npm_prefix/bin   ← 总是可写（用户或root权限）
   ├─ 尝试用户级别注册                  ← 无需sudo，大多数情况下成功
   └─ 如果失败 ➜ 提示用户运行 mcp-chrome-beta-bridge register --system
      └─ 需要手动使用管理员权限运行
```
...
```bash
mcp-chrome-beta-bridge register
```
...
```bash
# macOS/Linux
sudo mcp-chrome-beta-bridge register --system

# Windows (以管理员身份运行命令提示符)
mcp-chrome-beta-bridge register --system
```
...
```
sudo mcp-chrome-beta-bridge register
```
...
```
注册流程
├─ 用户级别注册 (mcp-chrome-beta-bridge register)
│  ├─ 获取用户级别清单路径
│  ├─ 创建用户目录
│  ├─ 生成清单内容
│  ├─ 写入清单文件
│  └─ Windows平台：创建用户级注册表项
│
└─ 系统级别注册 (mcp-chrome-beta-bridge register --system)
   ├─ 检查是否有管理员权限
   │  ├─ 有权限 → 直接创建系统目录和写入清单
   │  └─ 无权限 → 提示用户使用管理员权限运行
   └─ Windows平台：创建系统级注册表项
...
3. 如果没有管理员权限：
   - 提示用户使用管理员权限重新运行命令
   - macOS/Linux: `sudo mcp-chrome-beta-bridge register --system`
   - Windows: 以管理员身份运行命令提示符
...
│  │  └─ 运行 mcp-chrome-beta-bridge fix-permissions
│  │
│  └─ 尝试 mcp-chrome-beta-bridge register --system
│
├─ 路径问题
│  ├─ 检查Node.js安装 (node -v)
...
3. **修复执行权限问题**
   ...
   **解决方案**：
   ...
   ```bash
   # 查找安装路径
   npm list -g mcp-chrome-beta-bridge
   # 或者对于 pnpm
   pnpm list -g mcp-chrome-beta-bridge

   # 设置执行权限（替换为实际路径）
   chmod +x /path/to/node_modules/mcp-chrome-beta-bridge/run_host.sh
   chmod +x /path/to/node_modules/mcp-chrome-beta-bridge/index.js
   chmod +x /path/to/node_modules/mcp-chrome-beta-bridge/cli.js
   ```
   ...
   **解决方案**：
   ...
   ```bash
   # 卸载
   npm uninstall -g mcp-chrome-beta-bridge
   # 或 pnpm uninstall -g mcp-chrome-beta-bridge

   # 重新安装
   npm install -g mcp-chrome-beta-bridge
   # 或 pnpm install -g mcp-chrome-beta-bridge
...
5. 尝试使用系统级别安装
   - 使用 `mcp-chrome-beta-bridge register --system` 命令
   - 或直接使用管理员权限运行
